 ...
 *
 * โหมด "เปิดหลังคา" (`roofOpen`) เอาแผ่นหลังคา/ครีบสัน/สกายไลต์/หน่วยดาดฟ้า
 * ออก เหลือแค่โครงเหล็ก (โครงถักโค้ง คานอกไก่ แป) + ชายคา/รางน้ำ — ตรงกับ
 * พฤติกรรมของ `SiteEnvironment.tsx`/`buildLineHalls` ทุกประการ: สร้างเรขาคณิต
...
export interface HallRoofProps {
  layout: PlantLayout;
  /** true = เปิดหลังคา (ซ่อนแผ่นหลังคา เหลือแต่โครงถัก) เพื่อมองเห็นเครื่องจักรข้างใน */
  roofOpen?: boolean;
...
export function HallRoof({ layout, roofOpen = false, highQuality = true }: HallRoofProps) {
  ...
  //   - truss/eave: อยู่เสมอ ทั้งเปิด/ปิดหลังคา (โครงเหล็ก+ชายคา+รางน้ำ)
  ...
  //   - extra (ท่อน้ำฝน/ทางเดิน/ราวกันตก): หายเมื่อปิด highQuality เท่านั้น
  //     (ยังอยู่ตอนเปิดหลังคา ตามสเปก — ไม่ใช่ชิ้นส่วนหลังคาที่บังมุมมองด้านบน)

...
export function buildSiteBuildings(
  ...
  // ป้อมยามจริงมีชายคายื่น 0.4 ม./ด้าน เหมือนอาคารอื่นทุกหลังใน `RAW_BUILDINGS`
  // (`BUILDING_EAVE_OVERHANG`, siteShared.ts) — แถบ across ที่กินจริงบนจอจึง
  // กว้างกว่าที่คอมเมนต์นี้เคยระบุ (กล่องสำรวจเปลือย [-7.7,-2.3]) กลายเป็น
  // [-8.1,-1.9] เสาประตูฝั่งใกล้ (side=-1, across=6-7=-1, กว้าง 1.1 ม. ครึ่ง
  // 0.55 → ขอบเสาที่ across=-1.55) จึงห่างจากชายคาจริงแค่ -1.55-(-1.9) = 0.35 ม.
  // (เดิมคิดจากกล่องเปลือยได้ 0.75 ม.) ยังเป็นบวก ไม่ชนจริง แต่แคบกว่าที่เคย
  // เข้าใจ — ยังไม่ต้องขยับเพราะยังไม่ติดลบ
  const GUARDHOUSE_ACROSS = -5.0;
  const gateAlong = GUARDHOUSE_ALONG + 2; // ชิดป้อมยามจริง ไม่ต้องบีบแคบแล้ว
  const gateAcross = 6; // พ้นขอบป้อมยาม (ถึง -2.3 กล่องเปลือย / -1.9 รวมชายคา) ด้วยระยะเผื่อ >1 ม.
